...
export async function createTweetAsync(tweetText: string, authTokenSet: AuthTokenSet): Promise<any> {
    const CREATE_TWEET_QUERY_ID = "F7hteriqzdRzvMfXM6Ul4w";

    const endpoint = `/i/api/graphql/${CREATE_TWEET_QUERY_ID}/CreateTweet`;
    const transactionId = await getTransactionIdAsync("POST", endpoint);
    const referer = "https://x.com/compose/post";
    const headers = await createHeaderWithTransactionId(authTokenSet, referer, transactionId);
    ...
    Log.info('Creating tweet with text:', tweetText);

    const response = await fetch(
        `https://x.com${endpoint}`,
        {
            method: 'POST',
            headers,
    ...
    Log.info('end fetch create tweet');
    ...
    if (!response.ok) {
        return await handleTwitterApiError(response, authTokenSet.token, 'CreateTweet');
    }

    const responseText = await response.text();
    Log.info('CreateTweet API Response:', responseText);
